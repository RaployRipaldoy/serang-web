<div class="card">
    <div class="card-header">
        <h5 class="card-title mb-0">
            <i class="fas fa-edit"></i> Edit Jenis Perlengkapan
        </h5>
    </div>
    <div class="card-body">
        <!-- Alert Messages -->
        ...
        <form action="<?= base_url('DataJenisPerlengkapan/update/' . $jenisPerlengkapan['id']) ?>" method="post">
            <?= csrf_field() ?>
            ...
            <div class="row">
                <div class="col-md-8">
                    ...
                    <div class="alert alert-info">
                        ...
                        <table class="table table-sm table-borderless mb-0">
                            <tr>
                                <td width="30%">ID</td>
                                <td width="5%">:</td>
                                <td><?= esc($jenisPerlengkapan['id']) ?></td>
                            </tr>
                            <tr>
                                <td>Dibuat</td>
                                <td>:</td>
                                <td><?= date('d/m/Y H:i', strtotime($jenisPerlengkapan['created_at'])) ?> 
                                    oleh <?= esc($jenisPerlengkapan['created_by'] ?? 'System') ?></td>
                            </tr>
                            <tr>
                                <td>Terakhir Diubah</td>
                                <td>:</td>
                                <td><?= date('d/m/Y H:i', strtotime($jenisPerlengkapan['updated_at'])) ?> 
                                    oleh <?= esc($jenisPerlengkapan['updated_by'] ?? 'System') ?></td>
                            </tr>
                        </table>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" 
                               ...
                               value="<?= old('nama_perlengkapan', $jenisPerlengkapan['nama_perlengkapan']) ?>"
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" 
                               ...
                               value="<?= old('jenis_perlengkapan', $jenisPerlengkapan['jenis_perlengkapan']) ?>"
                ...
                <div class="col-md-4">
                        ...
                        <div class="card-body">
                            ...
                            <small class="text-muted">
                                <ul class="mb-0">
                                    ...
                                    <li>Timestamp akan otomatis diperbarui</li>
                                </ul>
                            </small>
                        </div>
                    </div>
                    ...
                    <!-- Change Detection -->
                    <div class="card bg-warning bg-opacity-10 mt-3" id="changeDetection" style="display: none;">
                        <div class="card-body">
                            <h6 class="card-title text-warning">
                                <i class="fas fa-exclamation-triangle"></i> Perubahan Terdeteksi
                            </h6>
                            <small id="changeList"></small>
                        </div>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const originalData = {
        nama_perlengkapan: '<?= esc($jenisPerlengkapan['nama_perlengkapan']) ?>',
        jenis_perlengkapan: '<?= esc($jenisPerlengkapan['jenis_perlengkapan']) ?>'
    };
    ...
    function checkChanges() {
        ...
        fields.forEach(function(fieldName) {
            ...
            if (currentValue !== originalValue) {
                ...
                changes.push(`<li>${label}: "${originalValue}" → "${currentValue}"</li>`);
            }
        });
        
        if (changes.length > 0) {
            changeList.innerHTML = '<ul class="mb-0">' + changes.join('') + '</ul>';
            changeDetection.style.display = 'block';
            saveBtn.className = 'btn btn-warning';
            saveBtn.innerHTML = '<i class="fas fa-save"></i> Simpan ' + changes.length + ' Perubahan';
        } else {
            changeDetection.style.display = 'none';
    ...
    window.resetForm = function() {
        fields.forEach(function(fieldName) {
            document.getElementById(fieldName).value = originalData[fieldName];
        });
        checkChanges();
    ...
    form.addEventListener('submit', function(e) {
        ...
        const hasChanges = fields.some(function(fieldName) {
            const field = document.getElementById(fieldName);
            return field.value.trim() !== originalData[fieldName];
        });
